...
OUT = os.path.join(BASE, "PowerBI_Course_App_v6.html")
...
RADIOS = "\n".join(
    ...
    + ['<input class="tabr" type="radio" name="tabc" id="r-%s">' % sid for _, sid, _, _, _ in SECTIONS + EXCEL_SECTIONS + PQ_SECTIONS]
)
...
for _sid in ["home"] + [sid for _, sid, _, _, _ in SECTIONS + EXCEL_SECTIONS + PQ_SECTIONS]:
...
ALL_SECTIONS = SECTIONS + EXCEL_SECTIONS + PQ_SECTIONS
for idx, (fname, sid, label, icon, desc) in enumerate(ALL_SECTIONS):
    ...
    if sid == "roadmap":
        nav_items.append('<li class="navgroup">⚡ POWER BI TRACK</li>')
    if sid == "x1":
        nav_items.append('<li class="navgroup">📗 EXCEL TRACK</li>')
    if sid == "p1":
        nav_items.append('<li class="navgroup">🧹 POWER QUERY TRACK</li>')
    nav_items.append(
        '<li data-s="%s"><label class="navlink" for="r-%s"><span class="ni">%s</span><span class="lbl">%s</span>'
        '<span class="tick" data-tick="%s"></span></label></li>' % (sid, sid, icon, html.escape(label), sid))
...
<div class="hero">
 ...
 <div class="hero-in">
  ...
  <p>12 Power BI + 5 Excel + 4 Power Query deep-dive modules · 63 auto-graded quiz questions · a 1,300-row ShopKart India dataset ·
...
<div class="stats">
 <div class="stat"><b data-count="21">0</b><span>Sections</span></div>
 <div class="stat"><b data-count="25">0</b><span>Hands-on labs</span></div>
 <div class="stat"><b data-count="63">0</b><span>Quiz questions</span></div>
...
<div id="certWrap"><div class="cert">
 ...
 21 sections covering Excel mastery, Power Query (ETL + M language), Data Modeling, DAX,<br>
 Visualization, Power BI Service, Performance Engineering and 3 portfolio capstone projects.</p>
