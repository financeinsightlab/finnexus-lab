# Builds PowerBI_Course_App.html v2 — premium interactive course app.
...
# ---------------- build ----------------
parts, home_cards, nav_items, readtimes = [], [], [], {}
for fname, sid, label, icon, desc in SECTIONS:
    ...
    parts.append(
        ...
        "</section>" % (sid, html.escape(label), icon, html.escape(label), rt, sid, body, quizzone, sid))
    home_cards.append(
        '<div class="card" data-goto="%s"><span class="cicon">%s</span><b>%s</b><small>%s</small>'
        '<span class="cdone" data-cdone="%s"></span></div>' % (sid, icon, label, desc, sid))
    nav_items.append(
...
section.lesson.show{display:block;animation:fadein .35s cubic-bezier(.2,.7,.3,1)}
...
.card{position:relative;background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:18px;cursor:pointer;transition:.18s;overflow:hidden}
...
// ---------- nav ----------
function go(id){if(!document.getElementById(id))id='home';
 ...
 try{window.scrollTo({top:0})}catch(e){}document.querySelector('aside').classList.remove('open');
 if(id!=='home'){last=id;ls('pbi2-last',id);buildToc(id);}else{$('#tocbox').innerHTML=''}
 paint();}
...
$all('.card').forEach(function(c){c.addEventListener('click',function(){location.hash=c.getAttribute('data-goto')})});
...
IDS.forEach(function(id,ix){var s=document.getElementById(id);var f=document.createElement('div');f.className='pn';
 ...
 s.appendChild(f);});
try{go(location.hash.slice(1)||'home');paint();}
catch(e){
 // last-resort fallback: unhide everything so content is never invisible
 try{$all('.lesson').forEach(function(s){s.classList.add('show')});}catch(e2){}
}
"""
